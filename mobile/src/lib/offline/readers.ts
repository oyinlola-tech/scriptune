import type { BookDto, ChapterDto, HymnDetailDto, Stanza, TranslationSummaryDto, VerseDetailDto, VerseDto } from "@scriptune/contracts";
import { openDatabase } from "./database";

interface BookRow { ord: number; slug: string; name: string; local_name: string | null; abbreviation: string; testament: "OLD" | "NEW"; deuterocanonical: number; chapter_count: number }
interface CorpusRow { title: string; rights_status: string; language: string; notice: string | null }
...
function toSummary(code: string, corpus: CorpusRow): TranslationSummaryDto {
  return { code, name: corpus.title, language: corpus.language, rightsStatus: corpus.rights_status, notice: corpus.notice };
}

function toBookDto(row: BookRow): BookDto {
  return { slug: row.slug, name: row.name, localName: row.local_name, abbreviation: row.abbreviation, testament: row.testament, deuterocanonical: row.deuterocanonical === 1, order: row.ord, chapterCount: row.chapter_count };
}

function toVerseDto(book: BookRow, row: VerseRow): VerseDto {
  return { reference: `${book.name} ${row.chapter}:${row.verse}`, book: { slug: book.slug, name: book.name, localName: book.local_name, abbreviation: book.abbreviation }, chapter: row.chapter, verse: row.verse, text: row.text };
}
...
async function findBook(translation: string, book: string): Promise<BookRow | null> {
  ...
  return db.getFirstAsync<BookRow>(
    "SELECT ord, slug, name, local_name, abbreviation, testament, deuterocanonical, chapter_count FROM books WHERE translation = ? AND (slug = ? OR lower(name) = ? OR lower(abbreviation) = ?)",
...
export async function readLocalVerse(translationCode: string, bookRef: string, chapter: number, verse: number): Promise<VerseDetailDto | null> {
  ...
  const db = await openDatabase();
  const corpus = await db.getFirstAsync<CorpusRow>("SELECT title, rights_status, language, notice FROM corpora WHERE kind = 'bible' AND id = ?", translation);
  ...
  return {
    translation: toSummary(translation, corpus),
...
export async function listLocalBooks(translationCode: string): Promise<{ translation: TranslationSummaryDto; books: BookDto[] } | null> {
  const translation = translationCode.toUpperCase();
  const db = await openDatabase();
  const corpus = await db.getFirstAsync<CorpusRow>("SELECT title, rights_status, language, notice FROM corpora WHERE kind = 'bible' AND id = ?", translation);
  if (corpus === null) return null;
  const rows = await db.getAllAsync<BookRow>("SELECT ord, slug, name, local_name, abbreviation, testament, deuterocanonical, chapter_count FROM books WHERE translation = ? ORDER BY ord", translation);
  return {
    translation: toSummary(translation, corpus),
