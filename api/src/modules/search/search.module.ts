...
import { APP_VERSION, SEARCH_RATE_LIMIT_MAX, TOKENS } from "../../constants/index.js";
import { createRateLimitMiddleware, type RateLimitMiddleware } from "../../middlewares/index.js";
...
export class SearchModule extends BaseModule {
  ...
  public override async onInitialize(): Promise<void> {
    ...
    this.rateLimit = createRateLimitMiddleware({ max: SEARCH_RATE_LIMIT_MAX, windowMs: 60_000 });
    registerRoutes(
      this.container.resolve(TOKENS.httpRouter),
      this.container.resolve(TOKENS.openApi),
      createSearchRoutes(new SearchController(queryBus), { rateLimit: this.rateLimit.middleware }),
    );
  }
}
